...
3. 配置 B2 的跨域，B2 跨域配置比较麻烦，需注意, 而端点 URL 在下图的 Endpoint 位置右边
   ![B2cors](/images/guide/B2/B2-3.png)
4. 可以先尝试一下 1 或 2，去到上传页面看看是否能上传，F12 打开控制台若显示跨域错误，则使用 3。要一劳永逸就直接使用 3（现在建议直接使用 3）。
...
关于腾讯云 COS 的密钥获取方法和跨域配置同理，不过多赘述。

如果想 COS 存储桶配置中添加你的自定义域名，但没有备案可参考下方 Cloudfare worker 反代配置
...
1. 若没有 TeBi Cloud 账号，可以先[注册](https://client.tebi.io/)一个（注意：需要绑卡验证，但不扣费），然后创建一个存储桶（免费额度 25GB/月，流量 25GB/月）
...
3. 端点 URL: `https://s3.tebi.io`

4. 配置跨域规则：点击存储桶右边进入 EDIT，找到 CORS，然后 Enable 打上 √，跨域规则配置参考下面代码，最后记得划到下面点击 UPDATE
...
1. 若没有七牛云账号，可以先[注册](https://sso.qiniu.com/)一个（注意：需要实名验证，极为严格），然后在[七牛云 - 对象存储 - 概览](https://portal.qiniu.com/kodo/overview)创建一个存储桶，存储区域没有特别要求，可以选择海外搭配海外加速域名

![](/images/guide/Kodo/Kodo-1.png)

2. 在[七牛云 - 密钥管理](https://portal.qiniu.com/developer/user/key)获取密钥（上面是 ID，下面是密钥）

3. 端点 URL 可在[服务域名使用指南](https://developer.qiniu.com/kodo/4088/s3-access-domainname)实时更新，或参考下表：

| 存储区域                | 区域简称 Region ID | 访问 Endpoint                 | 协议        |
| :---------------------- | :----------------- | :---------------------------- | :---------- |
| 华东-浙江               | cn-east-1          | s3.cn-east-1.qiniucs.com      | HTTP，HTTPS |
| 华东-浙江 2             | cn-east-2          | s3.cn-east-2.qiniucs.com      | HTTP，HTTPS |
| 华北-河北               | cn-north-1         | s3.cn-north-1.qiniucs.com     | HTTP，HTTPS |
| 华南-广东               | cn-south-1         | s3.cn-south-1.qiniucs.com     | HTTP，HTTPS |
| 西北-陕西 1             | cn-northwest-1     | s3.cn-northwest-1.qiniucs.com | HTTP，HTTPS |
...
4. 配置跨域规则：点击存储桶进入，找到空间设置，往下滑找到跨域设置，点击新增规则
...
::: warning 服务提醒
这家页面确实挺缤纷，但是送 50GB，并且实名验证好像不怎么严格，有可能跑路，建议不要存储珍贵资料。
:::

1. 若没有缤纷云账号，可以先[注册](https://console.bitiful.com/login)一个（注意：需要实名验证，应该随便一张身份证都能过），然后在[缤纷云控制台](https://console.bitiful.com/buckets)创建一个存储桶

2. 在[缤纷云控制台 - 密钥管理](https://console.bitiful.com/accessKey)获取密钥，先点击上面的添加用户，然后点击下面添加 Key，Access Key 是密钥 ID，Secret Key 是密钥

![](/images/guide/Bitiful/Bitiful-1.png)

3. 端点 URL: `https://s3.bitiful.net`，详细信息可在 `https://console.bitiful.com/buckets/你的存储桶的名称/baseinfo` 查看

4. 配置跨域规则：点击存储桶进入访问管理，找到跨域配置，点击新建规则，访问地址：`https://console.bitiful.com/buckets/你的存储桶的名称/ad`，跨域规则配置参考下图，最后点击保存
